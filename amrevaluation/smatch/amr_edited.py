...
class AMR(object):
    ...
    def get_triples(self):
        ...
        for i in range(len(self.nodes)):
            instance_triple.append(
                ("instance", self.nodes[i], self.node_values[i]))
            ...
            if c == "\"":
                ...
                # or node value (variable) at this moment
                    ...
                    if relation_name.endswith("-of") and normalize_inv:
                        allrelations.append(
                            (relation_value, relation_name[:-3], stack[-1]))
                        if relation_value not in node_dict:
                            node_relation_dict2[relation_value].append(
                                (relation_name[:-3], stack[-1]))
                        else:
                            node_relation_dict1[relation_value].append(
                                (relation_name[:-3], stack[-1]))
                        reent.append(
                            (relation_value, relation_name[:-3], stack[-1]))
                    else:
                        allrelations.append(
                            (stack[-1], relation_name, relation_value))
                        if relation_value not in node_dict:
                            node_relation_dict2[stack[-1]
                                                ].append((relation_name, relation_value))
                        else:
                            node_relation_dict1[stack[-1]
                                                ].append((relation_name, relation_value))
                        reent.append(
                            (stack[-1], relation_name, relation_value))
                state = 2
                ...
                if state == 1:
                    ...
                    cur_charseq[:] = []
                    # if this node name is already in node_dict, it is
                    # duplicate
                    ...
                    if cur_relation_name != "":
                        ...
                        # If the relation name ends with "-of", we store the
                        # reverse relation.
                        if (not cur_relation_name.endswith(
                                "-of")) or normalize_inv == False:
                            # stack[-2] is upper_level node we encountered, as
                            # we just add node_name to stack
                            node_relation_dict1[stack[-2]
                                                ].append((cur_relation_name, node_name))
                            allrelations.append(
                                (stack[-2], cur_relation_name, node_name))
                            # if node_name in node_name_list:
                            #	reent.append((stack[-2],cur_relation_name,node_name))
                        else:
                            # cur_relation_name[:-3] is to delete "-of"
                            node_relation_dict1[node_name].append(
                                (cur_relation_name[:-3], stack[-2]))
                            allrelations.append(
                                (node_name, cur_relation_name[:-3], stack[-2]))
                ...
                if len(stack) == 0:
                    print >> ERROR_LOG, "Unmatched parenthesis at position", i, "in processing", line[
                        0:i + 1]
                    return None
                # Last significant symbol is ":". Now we encounter ")"
                # Example:
                ...
                if state == 2:
                    ...
                    parts = temp_attr_value.split()
                    if len(parts) < 2:
                        print >> ERROR_LOG, "Error processing", line[:i +
                                                                     1], temp_attr_value
                        return None
                    relation_name = parts[0].strip()
                    relation_value = parts[1].strip()
                    # store reverse of the relation
                    # we are sure relation_value is a node here, as "-of"
                    # relation is only between two nodes
                    if relation_name.endswith("-of") and normalize_inv:
                        allrelations.append(
                            (relation_value, relation_name[:-3], stack[-1]))
                        node_relation_dict1[relation_value].append(
